...
class lab6
{
    static async Task Main(string[] args)
    {
        for (int j = 0; j < 10; j++)
        {
            ...
            List<Weather> weatherData = new List<Weather>(); // Создается список weatherData для хранения объектов погоды типа Weather.
            Random random = new Random(); //Создается экземпляр класса Random, предназначенный для генерации случайных чисел.

            for (int i = 0; i < 50; i++) //Этот цикл выполнится 50 раз, каждая итерация представляет собой получение данных погоды для случайной локации.
            {
                Console.Write($"{i} - ");
                double latitude = random.NextDouble() * (90 - (-90)) + (-90);
                double longitude = random.NextDouble() * (180 - (-180)) + (-180);

                Weather weather = await GetWeatherData(apiUrl, apiKey, latitude, longitude); //Здесь вызывается асинхронный метод GetWeatherData,
                                                                                             //который, предположительно, отправляет запрос к API сервиса погоды,
                                                                                             //используя заданный URL, ключ API, широту и долготу,
                                                                                             //и получает данные о погоде для указанной локации.
                if (weather != null)
                {
                    weatherData.Add(weather); //В случае успешного получения данных о погоде объект weather добавляется в список weatherData.
                }
            }
